...
const getEvent = (config: HLTVConfig) => async ({ id }: { id: number }): Promise<FullEvent> => {
    ...
    const mapPool = toArray($('.map-pool-map-holder')).map(mapEl => getMapSlug(mapEl.find('.map-pool-map-name').text()))

    return { name, dateStart, dateEnd, prizePool, teams, location, prizeDistribution, formats, relatedEvents, mapPool }
}
