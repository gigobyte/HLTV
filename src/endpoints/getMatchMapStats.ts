...
export interface MapStatsPerformanceOverview {
...
export interface FullMatchMapStats {
  ...
  performanceOverview: MapStatsPerformanceOverview
}
...
export const getMatchMapStats = (config: HLTVConfig) => async ({
  ...
  const teamStats = m$('.match-info-row')
    .toArray()
    .slice(1)
    .reduce((res, el, i) => {
  ...
  const mostX = m$('.most-x-box')
    ...
    .reduce((res, el, i) => {
      ...
      res[prop] = {
        ...
        name: m$('.most-x-box').eq(i).find('.name > a').text(),
        value: m$('.most-x-box').eq(i).find('.valueName').text()
      }
  ...
  const performanceOverview = p$('.overview-table tr')
    .toArray()
    .slice(1)
    .reduce(
      (res, el) => {
      ...
      { team1: {}, team2: {} } as MapStatsPerformanceOverview
    )
  ...
  return {
    ...
    team2,
    event,
    overview: { ...teamStats, ...mostX } as any,
    roundHistory,
    playerStats,
    performanceOverview
  }
...
function getOverviewPropertyFromLabel(
  label: string
