...
export const mapRoundElementToModel = (team1Id: number, team2Id: number) => (
  ...
  const outcome = Object.entries(Outcome).find(
    ([_, v]) => v === outcomeString
...
export const getMapsStatistics = (
  source: string
): { [key: string]: MapStatistic } | undefined => {
  ...
  const maps = (source.match(labelRegex) as RegExpMatchArray).map(
    x => x.split(':"')[1]
  );
